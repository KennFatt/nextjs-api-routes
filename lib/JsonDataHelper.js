...
function updateRecord(data, oldRecordId, newRecord) {
  ...
  if (oldRecordIndex === -1) {
    throw new Error(
      `The record with an \`id\` of ${oldRecordId} does not exist!`
    );
  }

  /** Deep comparison */
  let isDataUpdated = false;
  const oldRecordObject = data[oldRecordIndex];
  for (const k in oldRecordObject) {
    if (oldRecordObject[k] !== newRecord[k]) {
      isDataUpdated = true;
      break;
    }
  }
